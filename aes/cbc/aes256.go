...
func (aesCipher *AesCipher256) Encrypt(origData []byte) []byte {
	...
	newData := *aesCipher.BufferPool.Get(dataSize) // make([]byte, dataSize)
	defer aesCipher.BufferPool.Put(&newData)
...
func mcryptOpen() *AesCipher256 {
	td := &AesCipher256{BufferPool: pkg.NewLimitedPool(1, 4096)}
...
func mcryptGenericInit(td *AesCipher256, key, iv []byte) int {
	...
	if len(iv) < td.blockSize {
		newIv := *td.BufferPool.Get(td.blockSize) //make([]byte, td.blockSize)
		defer td.BufferPool.Put(&newIv)
		copy(newIv, iv)
		iv = newIv
	} else {
		iv = iv[:td.blockSize]
	}
	td.key = *td.BufferPool.Get(td.blockSize) //make([]byte, td.blockSize)
	defer td.BufferPool.Put(&td.key)
	copy(td.key, key)
